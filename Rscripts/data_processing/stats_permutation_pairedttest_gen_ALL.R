...
Analysis for GENUS-level.
T-test: one-tailed paired t-test (run first with alternative= 'greater' and then 'less') 
where the two inocula are NOT pooled together (pairing is done within each inoculum and they are kept separately). 
...
taxa.level<-'genus'
dsp<-select(dat, Inoculum, Genus, CS_type_duo, CS_duo, CS1_mono, CS2_mono, ab1_corr, ab2_corr, ab_pred, ab_duo,rep_mono1,rep_mono2,rep_duo)
dsp.s<-dsp
##.. Test whether GENUS X is significantly MORE  abundant than predicted by the null model (for each CS pair and inoculum)
...
for (i in 1:bs.rep) {
  ...
  #.. t-test by Inoculum
  t.test = pairing %>%
    dplyr::group_by(Inoculum, Genus, CS_duo) %>%
  ...
  t.test<-select(t.test, Inoculum, Genus, CS_type_duo, CS_duo, CS1_mono, CS2_mono, mean_diff, p.val, n, t.stat, t.altern, t.method,epsi_sd,epsi_se)
...
#  ---------------------------------------------------------------------------
##.. now do test for GLYCINE only (there are 3 replicates per CS not 4)
...
#  ---------------------------------------------------------------------------
##.. now test whether Genus X is significantly LESS abundant than predicted by the null model (for each CS pair and inoculum)
...
#.. and plot epsilon for Inoculum x Genus x CS pair
#.. NOTE: make sure to run both t-alternative= 'greater' and 'less' before running script below
#------

#.. only select pairs with n>3
d.lg<-subset(dp.all.lg, n>=3)
hist(d.lg$n)

#.. assign significance value for permutation test (depends on df:  https://www.sjsu.edu/faculty/gerstman/StatPrimer/t-table.pdf)
sig.v<-0.95
t.c4<-2.353 # t-test t value value assuming that df=3 (i.e. N=4-1) and cf=0.95
t.c3<-2.920 # t-test t value value assuming that df=2 (i.e. N=3-1) and cf=0.95

d.lg$t.critic <- with(d.lg, ifelse(n ==4, t.c4,
                                   ifelse(n==3, t.c3, NA)))
...
d.lg<- d.lg %>% 
  dplyr::group_by(Inoculum, Genus, CS_duo, CS_type_duo,CS1_mono,CS2_mono,t.altern) %>% 
  dplyr::summarize(
