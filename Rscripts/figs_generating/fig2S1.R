" R script for Figure 2- figure supplement 1 in Estrela et al (2021) 
...
#.. Make plot
#. select single carbon sources only
dp1<-subset(dfp, nCS ==1)

#. define order of CS for plot
levels(factor(dp1$Carbon_Source))
dp1$cs.ord <- factor(dp1$Carbon_Source, 
                     levels = c("glucose","fructose", "cellobiose", "ribose", "glycerol",
...
p1<-ggplot(dp1, aes(x=factor(Replicate), y=sum_relab, fill=Family_type))+
  ...
  scale_fill_manual(values=c(colAlc,colBruc,colChit,colCom,colEntbf,colEntc,colFlav,colLach,colMoraxf,colOxal,colPseuf,colRhizf,colSphing,colXanth, colOther))+
